...
- (instancetype)initWithId:(int) pathId strokeColor:(UIColor*) strokeColor strokeWidth:(int) strokeWidth {
    ...
    if (self) {
        ...
        _isTranslucent = CGColorGetComponents(strokeColor.CGColor)[3] != 1.0 &&
            ![RNImageEditorUtility isSameColor:strokeColor color:[UIColor clearColor]];
...
- (CGRect)addPoint:(CGPoint) point {
    ...
    if (_isTranslucent) {
        if (pointsCount >= 3) {
            [RNImageEditorUtility addPointToPath: _path
                            toPoint: point
            ...
            [RNImageEditorUtility addPointToPath: _path toPoint: point tertiaryPoint: point previousPoint: point];
        }
...
- (void)drawInContext:(CGContextRef)context pointIndex:(NSUInteger)pointIndex {
    ...
    BOOL isErase = [RNImageEditorUtility isSameColor:_strokeColor color:[UIColor clearColor]];
